 var globalBlockCount = 0;
     ...
     $(function () { 
       ...
       function BlockElement(w, h, l, t, b) {
          ...
          this.shake = function(block) {
               ...
               block.delay(150).animate({'top' : toMoveDown},10, function() {
                    block.animate({'top' : toMoveUp},10)
                    });
          }
          ...
          this.spawnBricks = function(block) {
               ...
               $('.mini_brick').css({'top':miniPosY, 'left' : miniPosX});
               $('.mini_brick').show(); 
               $('.mini_brick').each(function(e){
                    ...
                    var minBlockInt = setInterval(function(){
                         if(i <=5) {
                              ...
                              y = Math.pow(i, 4) * 4;
                         $this.css({'left' : miniPosX-x, 'top' : miniPosY+y});
     ...
     var meJumpInt = setInterval(function(){
          ...
          MARIO.css({'left' : xpos, 'top' : ypos});
          
          if(iInc == 20) { xA = xA; xY = -(xY)}
          if(iInc == 40) { xA = xA * .5; xY = 0; }
          if(iInc == 60) {
     ...
     function levelSwitch() {
          if(checkLevelInt == undefined) var checkLevelInt= setInterval(function(){
               ...
               MARIO.stop().animate({'top' : levelfloor },10, function(){ currentPlane = 300;});
       ...
       function activePipeCheck() {
          ...
          function transPipe(pT) {
               tpD = pT.position().top;
               $('embed').remove();
               $('body').append('<embed src="smb_pipe.wav" autostart="true" hidden="true" loop="false">');
               MARIO.css('z-index','-100');
               MARIO.addClass('crouching');
               MARIO.stop().animate({'top':tpD },1000,function(){
               MARIO.removeClass('crouching');
               MARIO.css('z-index',100);
               MARIO.animate({'left': 500, 'top' : 20},1,function(){ MARIO.animate({'top': 300},300);});
             ...
             if(event.which == 39 && jumpInProgress == false && MARIO.offset().top == currentPlane) {
                      ...
                      if(lastInc > 150) {
                              MARIO.animate({
                                   ...
                                   'step' : function(){if(MARIO.offset().top != currentPlane) $(this).stop(); lastInc = 0; }
                              }, 400, function () {
                                      lastInc = 0;
